...
def make_loader(dataset, batch_size, shuffle, num_workers=None, drop_last=False):
    ...
    with torch.no_grad():
        for mask_type, mask_ratio in config.VAL_PANEL:
            ...
            for batch in tqdm(dl_val, desc=f"val {mask_type}_{int(mask_ratio * 100)}%", leave=False):
                ...
                masks, actual_ratios = batch_sample_masks(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="实验三 Pixel-space MAE 训练")
