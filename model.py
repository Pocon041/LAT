...
def batch_sample_train_masks(batch_size, grid_size, device):
    ...
    meta = []
    for _ in range(batch_size):
        mask, actual_ratio, mask_type, ratio = sample_train_mask(grid_size)
        masks.append(mask)
        meta.append({
    ...
    return torch.stack(masks, dim=0).to(device), meta
...
class PixelMAE(nn.Module):
    ...
    def encode_visible(self, tokens, mask):
        ...
        for b in range(tokens.shape[0]):
            visible = tokens[b, ~mask[b]] + self.enc_pos_embed[0, ~mask[b]].to(tokens.dtype)
            visible_list.append(visible)
            lengths.append(visible.shape[0])
        padded = pad_sequence(visible_list, batch_first=True)
        key_padding_mask = torch.ones(
            padded.shape[0], padded.shape[1], dtype=torch.bool, device=padded.device
        )
        for b, length in enumerate(lengths):
            key_padding_mask[b, :length] = False
        encoded = self.encoder(padded, key_padding_mask=key_padding_mask)
        return encoded, lengths

    def decode_full(self, encoded_visible, lengths, mask):
        bsz = mask.shape[0]
        dec_tokens = self.mask_token.expand(bsz, self.num_tokens, -1).clone()
        for b in range(bsz):
            dec_tokens[b, ~mask[b]] = self.decoder_embed(encoded_visible[b, : lengths[b]])
