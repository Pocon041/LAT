...
def get_dataset(split, return_path=False):
    if split == "test":
        return ChameleonTestDataset(return_path=return_path, preload=config.TEST_PRELOAD_TO_RAM)
    if split == "val":
        class _ValDataset(RealPatchDataset):
            def __getitem__(self, idx):
                ...
                if return_path:
                    ...
                    return img, 0, path
                return img, 0
        return _ValDataset(split="val", patches_per_image=1, preload=config.VAL_PRELOAD_TO_RAM)
...
def visualize(args):
    ...
    for vis_idx, ds_idx in enumerate(indices):
        ...
        x = move_to_device(x.unsqueeze(0), device)
        ...
        axes[1, 0].imshow(core_arr, cmap="gray")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="实验三重建与误差可视化 (5090版)")
